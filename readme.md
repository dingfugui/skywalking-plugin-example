...
5，在resource目录创建文件：skywalking-plugin.def，输入foo-plugin=org.dfg.demo.sk.plugin.foo.define.Base64Instrumentation。

告诉skywalking加载哪个类

6，打包编译。由于插件开发依赖bytebuddy，所以需要把bytebuddy加入环境。官方会把bytebuddy放入自己的jar包中国，并用maven-shade-plugin插件，把插件代码中import的bytebuddy路径替换为skywalking包路径。当然也可以使用其他方式把bytebuddy的class repackage到插件jar中。

7，下载skywalking，建议在测试中使用h2版skywalking，默认数据源为h2，可以直接启动

8，将打包好的插件jar放入skywalking目录：{skywalking_home}\agent\plugins\
